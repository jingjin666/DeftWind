...
void RCOutput::init()
{
    ...
    for (uint8_t i=0; i < UAVRS_NUM_OUTPUT_CHANNELS; i++) {
 ...
 */
void RCOutput::set_freq_fd(int fd, uint32_t chmask, uint16_t freq_hz, uint32_t &rate_mask) 
{
...
{
    if (ch >= UAVRS_NUM_OUTPUT_CHANNELS) {
        return;
    }
