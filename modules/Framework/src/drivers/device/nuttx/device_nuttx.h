...
#define DEVICE_LOG(FMT, ...) DP_LOG_NAMED(_name, FMT, ##__VA_ARGS__)
#define DEVICE_DEBUG(FMT, ...) DP_LOG_NAMED_COND(_name, _debug_enabled, FMT, ##__VA_ARGS__)
...
protected:
	 ...
	 */
	virtual void	poll_notify_one(dp_pollfd_struct_t *fds, pollevent_t events);
...
private:
	 ...
	 */
	int		store_poll_waiter(dp_pollfd_struct_t *fds);
